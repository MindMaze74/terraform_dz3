# Домашнее задание к занятию «Управляющие конструкции в коде Terraform» - Старцев Данила Антонович
...
> |----------|-------------|
> | Переменная `token` не имела значения по умолчанию, и при каждом `terraform plan` запрашивался > ввод токена. | Перешли на аутентификацию через сервисный аккаунт. Удалили переменную `token` из  > `variables.tf` и строку `token = var.token` из `providers.tf`. Добавили >`service_account_key_file = "/home/user/authorized_key.json"`. |
>| Опечатка в `default_zone`: `"ru-central-a"` вместо `"ru-central1-a"`. | Исправлено на >`"ru-central1-a"`. |
>| После комментирования `token` в `variables.tf` остались «висячие» аргументы (`type`, >`description`), что вызывало синтаксическую ошибку. | Удалили весь блок переменной `token` >целиком. |
...
> [ссылка на отчет](https://github.com/MindMaze74/terraform_dz3/tree/terraform-03#дополнительные-задания-со-звездочкой)
> [ссылка на коммит b9f5fb9 что было изменено после выполнения 4 задания](https://github.com/MindMaze74/terraform_dz3/commit/b9f5fb9eea9bc42c68aa71080f5cec0fc2f1dad0)
